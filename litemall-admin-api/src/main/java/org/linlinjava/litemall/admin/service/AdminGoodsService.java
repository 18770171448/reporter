...
public class AdminGoodsService {
     ...
     * <p>
     * 这里文章三个表的数据采用删除再添加的策略是因为
     * 文章编辑页面，支持管理员添加删除文章规格、添加删除文章属性，因此这里仅仅更新是不可能的，
     * 只能删除三个表旧的数据，然后添加新的数据。
     * 但是这里又会引入新的问题，就是存在订单文章货品ID指向了失效的文章货品表。
     * 因此这里会拒绝管理员编辑文章，如果订单或购物车中存在文章。
     * 所以这里可能需要重新设计。
     */
    @Transactional
    public Object update(GoodsAllinone goodsAllinone) {
        ...
        // 文章规格表litemall_goods_specification
        for (LitemallGoodsSpecification specification : specifications) {
            specification.setGoodsId(goods.getId());
            specificationService.add(specification);
        }

        // 文章参数表litemall_goods_attribute
        for (LitemallGoodsAttribute attribute : attributes) {
            attribute.setGoodsId(goods.getId());
            attributeService.add(attribute);
        }

        // 文章货品表litemall_product
        for (LitemallGoodsProduct product : products) {
    ...
    public Object create(GoodsAllinone goodsAllinone) {
        ...
        if (goodsService.checkExistByName(name)) {
            return ResponseUtil.fail(GOODS_NAME_EXIST, "文章名已经存在");
        }
