<template>
  <div class="app-container">
    ...
    <el-form ref="topic" :rules="rules" :model="topic" status-icon label-position="left" label-width="100px" style="width: 800px; margin-left:50px;">
      ...
      <el-form-item label="导读标题" prop="subtitle">
      ...
      <el-form-item label="文章类型" prop="goods">
        <el-button style="float:right;" size="mini" type="primary" @click="handleCreate()">发布文章</el-button>

        <!-- 查询结果 -->
        <el-table :data="goodsList" border fit highlight-current-row>

          <el-table-column align="center" label="文章编号" prop="id"/>
          ...
          <el-table-column align="center" label="导语" prop="brief"/>
    ...
    <el-dialog :visible.sync="addVisiable" title="写文章">
      <div class="search">
        ...
        <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row @selection-change="handleSelectionChange">
          ...
          <el-table-column align="center" label="文章标题" prop="name"/>
        </el-table>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        subtitle: [
          { required: true, message: '导读标题不能为空', trigger: 'blur' }
        ],
        content: [
          { required: true, message: '文章内容不能为空', trigger: 'blur' }
        ],
        price: [
          { required: true, message: '专题评分不能为空', trigger: 'blur' }
        ]
      },
      editorInit: {
        language: 'zh_CN',
